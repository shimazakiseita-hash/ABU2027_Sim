...
「単一トピックで受ける」設計により、`/tr_cmd_vel`等の指令トピックへは意思決定
ノード(`br_decision_tr_node`等)以外もpublishできる。デバッグ用の手動操縦コンソール
`br_teleop_node`(`enable_teleop:=true`で起動)は、意思決定ノードと同じトピックへ
publishするだけの別プロセスであり、sim_bridge_node側は操縦元(自律/手動)を
意識しない。
