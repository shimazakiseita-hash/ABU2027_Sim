...
（アース2段+スカイ1段）を組み上げる一連の動作が自動的に進む
（所要時間は目安60〜90秒）。得点は`/score/red`・`/score/blue`
（`std_msgs/Int32`）に発行される。
...
別ターミナルで実行し、時間経過とともに 10 → 30 → 70 と増えていけば
正常に動作している（アース1段目・2段目・スカイの順）。
